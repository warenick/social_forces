...
class FullAttGoalPredictor(nn.Module):
    def __init__(self):
        ...
        self.q_hist_emb = nn.Sequential(nn.Linear(6, 32), nn.BatchNorm1d(8))
        self.k_hist_emb = nn.Sequential(nn.Linear(6, 32), nn.BatchNorm1d(8))
        self.v_hist_emb = nn.Sequential(nn.Linear(6, 32), nn.BatchNorm1d(8))
        self.hist_enc = torch.nn.MultiheadAttention(embed_dim=32, num_heads=4, dropout=0.3)
        self.att_emb = nn.Sequential(nn.Linear(32, 64), nn.BatchNorm1d(64))

        self.att = torch.nn.MultiheadAttention(embed_dim=64, num_heads=8, dropout=0.3)
        self.pose_embed = nn.Linear(6 * 8, 32)
        self.q_emb = nn.Sequential(nn.Linear(32, 64))
        self.k_emb = nn.Sequential(nn.Linear(32, 64))
        self.v_emb = nn.Sequential(nn.Linear(32, 64))

        self.att_dec = torch.nn.MultiheadAttention(embed_dim=64, num_heads=8, dropout=0.3)
...
class LSTM_simple(nn.Module):
    def __init__(self):
        ...
        self.hist_emb = nn.Sequential(nn.Linear(6, 32), nn.BatchNorm1d(8))
        # self.k_hist_emb = nn.Sequential(nn.Linear(6, 32), nn.BatchNorm1d(8))
        # self.v_hist_emb = nn.Sequential(nn.Linear(6, 32), nn.BatchNorm1d(8))
        # self.hist_enc = torch.nn.MultiheadAttention(embed_dim=32, num_heads=4, dropout=0.3)
        # self.att_emb = nn.Sequential(nn.Linear(32, 64), nn.BatchNorm1d(64))
